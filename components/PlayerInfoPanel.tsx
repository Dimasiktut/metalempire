  ...
  return (
    <aside className="hidden md:flex flex-col justify-center gap-6 h-full">
      {players.map((player) => {
        ...
        else if (isLocal) borderColor = '#3b82f6'; // A distinct color for the local player
        ...
        return (
          <div
            ...
            className={`w-52 p-4 rounded-lg shadow-lg border-2 transition-all duration-300 transform
              ...
              ${player.money < 0 ? 'bg-red-900/50' : 'bg-slate-800/80'}
            `}
            style={{
          ...
          >
            ...
            <div className="text-3xl font-light text-green-300">
              {player.money < 0 ? 'Банкрот' : `$${player.money.toLocaleString()}`}
            </div>
            {isLocal && <div className="text-center text-xs text-blue-400 mt-2 font-bold">(Это вы)</div>}
          </div>
        )
      })}
    </aside>
